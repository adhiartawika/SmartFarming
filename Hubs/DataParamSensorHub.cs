    ...
    public class DataParamSensorHub : Hub
    {
        ...
        public async Task JoinRoom(string roomId)
        {
            ...
            if (roomId.Contains("-_-iot"))//
            {
                ...
                var gh = this.appContext.Mikrokontrollers.Where(x => x.Id == id).Include(x => x.MiniPc).FirstOrDefault();
                if (gh != null)
                {
                    ...
                    await Clients.Group(gh.MiniPc.RegionId.ToString()).SendAsync("IoTChangeStatus", new IoTChangeStatusDto { Id = id, IsActive = true });
                }
        ...
        public async Task LeaveRoom(string roomId)
        {
            if (roomId.Contains("-_-iot"))
            {
                ...
                if (gh != null)
                {
                    ...
                    var tempEsps = this.appContext.Mikrokontrollers.Include(x => x.MiniPc).ThenInclude(x => x.Region).Where(x => x.Id == gh.MiniPc.RegionId).ToList();
                    ...
                    await this.appContext.SaveChangesAsync(new System.Threading.CancellationToken());
                    await Clients.Group(gh.MiniPc.RegionId.ToString()).SendAsync("IoTChangeStatus", new IoTChangeStatusDto { Id = id, IsActive = false });
                }
                DataParamSensorHub.connGroup.Add(Context.ConnectionId, id + "_gh");
                await Groups.AddToGroupAsync(Context.ConnectionId, id + "_gh");
            }
